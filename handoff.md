...
- ⚠️ Written in a Linux container without a Swift toolchain — needs `swift build` + `swift run HeardTests` on macOS CI to verify.
...
- Speaker table with inline rename, merge, delete (context menu), search, and sort (Name / Meetings / Time in Meetings / Last Seen); a leading **Voice** column has a play/stop button that replays the speaker's saved voice clip via a shared `SpeakerClipController` so only one clip plays at a time
